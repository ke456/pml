...
int main(int argc, char ** argv)
{
    ...
    if (argc!=4)
        throw std::invalid_argument("Usage: ./time_pmbasis nbits fftprime order");
    ...
    if (argc==4)
    {
        const long order = atoi(argv[3]);
        cout << "# order " << order << endl;

				for(long rdim = 8; rdim <= 512; rdim *= 2)
           one_bench_pmbasis(rdim,rdim/2,order-1,order);
    }
